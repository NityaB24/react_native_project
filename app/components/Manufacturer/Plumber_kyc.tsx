...
import { View, Text, StyleSheet, ActivityIndicator, ScrollView, Animated, Easing, TouchableOpacity, Image, Modal, Alert, TextInput } from 'react-native';
...
const KYCRequests: React.FC = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            if (fetchedToken) {
                const response = await axios.get('http://192.168.29.101:3000/api/userkyc/requests', {
                    headers: {
    ...
    const handleApproval = async (id: string) => {
        try {
            ...
            const response = await axios.post('http://192.168.29.101:3000/api/userkyc/approve', { id }, {
                headers: {
                    Authorization: `Bearer ${fetchedToken}`,
                },
            });
    ...
    const handleRejection = async (id: string, comment: string) => {
        try {
            ...
            const response = await axios.post('http://192.168.29.101:3000/api/userkyc/reject', { id, comment }, {
                headers: {
                    Authorization: `Bearer ${fetchedToken}`,
                },
            });
    ...
    return (
        <>
            ...
            <FontAwesome name="bars" size={30} color="#4caf50" />
      </TouchableOpacity>
