  ...
  const fetchSchemes = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        if (schemes.length > 0) {
          const { userScheme } = schemes[0]; // Access the first item of the array
          setImageUris({
  ...
  return (
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.header}>Plumber Scheme</Text>
        {imageUris.userScheme && (
          <View style={styles.previewContainer}>
            {imageUris.userScheme ? (
                <TouchableOpacity onPress={() => openFullScreenImage(imageUris.userScheme)}>
              <Image source={{ uri: imageUris.userScheme }} style={styles.images} />
              </TouchableOpacity>
            ) : null}
          </View>
        )}
